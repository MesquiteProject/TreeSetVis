...
public abstract class PointSetVisualization extends MesquiteWindow {
	...
	NumberForTree treeScoresTask=null;
	...
	protected MesquiteBoolean treesColoredByScore = new MesquiteBoolean(false);

	/** commands to be able to put event handling on main Mesquite execution thread */
	//	private MesquiteCommand treeScoreColoringCheckboxCommand;
	private MesquiteCommand treesColoredByScoreCommand;
	private MesquiteCommand animateButtonCommand, animationRateFieldCommand, sampleCheckboxCommand, sampleSizeFieldCommand, decStepSizeButtonCommand, incStepSizeButtonCommand, stepSizeFieldCommand, scrambleButtonCommand, startStopButtonCommand;
	...
	TreeSetVisualization ownerModule = null;
	...
	public PointSetVisualization(MesquiteModule ownerModule, int numItems) {
		...
		if (ownerModule instanceof TreeSetVisualization) {
			this.ownerModule = (TreeSetVisualization)ownerModule;
			pointSize = this.ownerModule.getPointSize();
			frameDot = this.ownerModule.getFrameDot();
		}
		...
		embeddingDisplay = new EmbeddingDisplayPanel(selectionManager, sharedPoints, this, this.ownerModule);
		selectionManager.setEmbeddingDisplay(embeddingDisplay);
		selectionManager.setColorKey(null);

//thread had been started here; dangerous, because makes reference to fields not yet initialized here
		 ...
		 */
		treesColoredByScoreCommand = new MesquiteCommand("setTreesColoredByScore", this);
		//		treeScoreColoringCheckboxCommand = new MesquiteCommand("setTreeScoreColoringCheckBox", this);
		...
		treeScoreColoringCheckbox.addItemListener(new ItemListener() {
			public void itemStateChanged(ItemEvent e) {
				if (treeScoreColoringCheckbox.getState())
					treesColoredByScoreCommand.doItMainThread("on", null, null);
				else
					treesColoredByScoreCommand.doItMainThread("off", null, null);
			}
		});
		...
		if (false && MesquiteThread.isScripting()){
			mdsThread.run();
		}
		else {  //see below if scripting
	...
	public Snapshot getSnapshot(MesquiteFile file) {
		...
		windowSnapshot.addLine("suppressEPC");
		...
		windowSnapshot.addLine("setTreesColoredByScore " + treesColoredByScore.toOffOnString());
		...
		windowSnapshot.addLine("calcScoresIfNeeded");
		return windowSnapshot;
	}

	/* ................................................................................................................. */
	public void numberForTreeParamsChanged() {
		needToRecalculateTreeScores = true;
		refreshTreeScores();
	}
	/* ................................................................................................................. */
	public void resetBackgrounds() {
		embeddingDisplay.resetBackgrounds();
	...
	public Object doCommand(String commandName, String arguments, CommandChecker checker) {
		...
		if (checker.compare(this.getClass(), "Starts or stops the MDS", "[start or stop]", commandName, "startStopButtonCommand")) {
		...
		else if (checker.compare(this.getClass(), "Calculates tree scores", null, commandName, "calcScoresIfNeeded")) {
			ownerModule.suppressEPC = false;
			refreshTreeScores();
		}
		else if (checker.compare(this.getClass(), "Calculates tree scores", null, commandName, "suppressEPC")) {
			ownerModule.suppressEPC = true;
		}
			...
			if (StringUtil.notEmpty(arguments)) {
				NumberForTree temp = (NumberForTree)getOwnerModule().replaceEmployee(NumberForTree.class, arguments, "Values by which to color trees", treeScoresTask);

				if (temp != null) {
					embeddingDisplay.setTreeScoreColoring(false);
					embeddingDisplay.repaint();

					treeScoresTask = temp;
					treeScoresTaskName.setValue(treeScoresTask.getName());
					needToRecalculateTreeScores = true;
					treesColoredByScore.setValue(true);
					treeScoreColoringCheckbox.setState(true);
					if (!MesquiteThread.isScripting())
						refreshTreeScores();
					return treeScoresTask;
				}
			}
		}
		else if (checker.compare(this.getClass(), "turns tree score coloring on or off", "[on; off]", commandName, "setTreesColoredByScore")) {
			boolean treeScoreWasOn = treesColoredByScore.getValue();
			boolean treeScoreOn = MesquiteBoolean.fromOffOnString(parser.getFirstToken(arguments));
			if (treeScoreColoringCheckbox!=null)
				treeScoreColoringCheckbox.setState(treeScoreOn);
			treesColoredByScore.setValue(treeScoreOn);

			if (treesColoredByScore.getValue()) {   
				if (treeScoresTask == null){
					treeScoresTask = (NumberForTree)getOwnerModule().hireEmployee( NumberForTree.class, "Values by which to color tree");

				}
				if (treeScoresTask == null){
					treesColoredByScore.setValue(false);
					if (treeScoreColoringCheckbox!=null) 
		...
		/*		else if (checker.compare(this.getClass(), "turns tree score coloring on or off", "[on; off]", commandName, "setTreeScoreColoringCheckBox")) {
			boolean treeScoreWasOn = treeScoreColoringCheckbox.getState();
			boolean treeScoreOn = MesquiteBoolean.fromOffOnString(parser.getFirstToken(arguments));
	...
	public void refreshTreeScores(){
		if (treesColoredByScore.getValue()) {
			if ((needToRecalculateTreeScores || treeScoreColors == null) && treeScoresTask != null) {// This is the first time coloring has been activated
				calculateTreeScores(itemDiffMatrix.getNumberOfItems());  

				if (treeScores != null) {
					if (colorKey != null)
						mainPanel.remove(colorKey);
					colorKey = new ColorKey(treeScores, colorGradient, selectionManager, this);
	...
	protected abstract void newSelection(java.util.BitSet selection, String selectionName, int selectionNumber);

	protected abstract void updateSelection(java.util.BitSet selection, String selectionName, int selectionNumber);

	protected abstract void removeConsensusWindow(String selectionName, int selectionNumber);

	protected void removeSelection(int sel){ //added by WPM Sept 09 a because method above doesn't complete the task
	...
	protected abstract void calculateTreeScores(int numberOfScores);
...
class ColorKey extends Panel {
	...
	public ColorKey(double[] treeScores, ColorGradient colorGradient, SelectionManager selectionManager, PointSetVisualization mainWindow) {
	...
	public void paint(Graphics g) {
		...
		for (int i = 0; i < numberOfLabels; ++i) {
			...
			if (numberOfLabels>1)
				currentLabel = Double.toString(min + (i * (max - min) / (numberOfLabels-1)));  //DRM 14 May 2009:  changed to numberofLabels-1
			else
		...
		for (int i = 0; i <= lastShown; ++i) {
			if (arrowSet.get(i)) {
				...
				arrow.translate(x, y);
				g2.fill(arrow);
				PointIcons.draw(iconNumber,g2,new Point(arrowMargin + iconWidth/2,y),mainWindow.getPointSize(), mainWindow.getFrameDot());
				arrow.translate(-x, -y);
				numberOfArrowsDrawn++;
...
class EmbeddingDisplayPanel extends Panel {
	...
	private  Color embeddingBackGroundColor = Color.black;
	private  Color embeddingUnselectedPointColor = Color.white;
	private final static Color embeddingSelectedPointColor = Color.red;
	private  Color embeddingSelectionBoxColor = Color.white;
	...
	private int lastShown;
	TreeSetVisualization ownerModule;

	public EmbeddingDisplayPanel(SelectionManager selectionManager, SharedPoints sharedPoints, PointSetVisualization mainWindow, TreeSetVisualization ownerModule) {
		super();
	...
	public void resetBackgrounds() {
		if (ownerModule!=null) {
			embeddingBackGroundColor = ownerModule.getBackGroundColor();
			embeddingUnselectedPointColor = ownerModule.getUnselectedPointColor();
			embeddingSelectionBoxColor = ownerModule.getSelectionBoxColor();
		}
	}
	...
	private final void drawPointSet(BitSet b, int iconNumber, Color c, Graphics g) {
		...
		for (int i=0; i <= lastShown; ++i) {
			if (b.get(i)) {
				...
				PointIcons.draw(iconNumber,g,localPoints[i],mainWindow.getPointSize(), mainWindow.getFrameDot());
			}
		}
	}
...
class SelectionManager extends Panel implements Commandable {
	private MultiSelections ms;
	private EmbeddingDisplayPanel embeddingDisplay;
	private ColorKey colorKey;
	Component selectionDisplay;
	ScrollPane selectionPane;
	private static final int selectionBoxWidth = 130;
	private static final int selectionBoxHeight = 15;
	private PointSetVisualization mainWindow;
	MesquiteCommand removeActiveSelectionCommand, addSelectionCommand;

	public SelectionManager(int numberOfPoints, PointSetVisualization mainWindow) {
		this.mainWindow = mainWindow;
		ms = new MultiSelections(numberOfPoints,mainWindow, this);
		removeActiveSelectionCommand = new MesquiteCommand("removeActive", this);
		addSelectionCommand = new MesquiteCommand("addSelection", this);
		...
		addSelectionButton.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				addSelectionCommand.doItMainThread(e.getActionCommand(), null, null);
		...
		removeSelectionButton.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				removeActiveSelectionCommand.doItMainThread(e.getActionCommand(), null, null);
			}
		});
	...
	public void removeSelection(int sel){
		ms.setActiveSelection(sel);
		removeActiveSelection();
	}
	public void removeActiveSelection(){
		if (ms.getNumberOfSelections() > 1) {
			ms.removeActiveSelection();
	...
	public void resetNumberOfItems(int newNumberOfItems, PointSetVisualization mainWindow) {
		...
		ms = new MultiSelections(newNumberOfItems, mainWindow, this);
	}
	...
	public void selectionEvent(BitSet selectionSet, int modifierKeys) {
		...
		if (!selectionSet.equals(ms.getSelection(ms.getActiveSelection()))) { //the selection has changed
			embeddingDisplay.selectionSet = selectionSet;
			//WPM Aug09 routed this via standard main Mesquite execution thread, as per Mesquite convention
			(new MesquiteCommand("captureSelection", this)).doItMainThread(null, null, null);
		}
	}//selectionEvent
	public Object doCommand(String commandName, String arguments, CommandChecker checker) {
		if (checker.compare(this.getClass(), "Responds to UI change in selection", null, commandName, "captureSelection")) {
			//update selection data structure
			ms.setSelection(ms.getActiveSelection(),embeddingDisplay.selectionSet);
		...
		else if (checker.compare(this.getClass(), "Removes active selection", null, commandName, "removeActive")) {
			removeActiveSelection();
		}
		else if (checker.compare(this.getClass(), "Adds selection", null, commandName, "addSelection")) {
			addSelection();
		}
		return null;
	}
	...
	private void drawSelection(boolean active, int iconNumber, int userNumber, int numberOfPoints, int x, int y, Graphics2D g2) {
		...
		g2.setRenderingHint(RenderingHints.KEY_ANTIALIASING, RenderingHints.VALUE_ANTIALIAS_ON);
		PointIcons.draw(iconNumber,g2,new Point(x+9,y+selectionBoxHeight/2),mainWindow.getPointSize(), mainWindow.getFrameDot());
		g2.setRenderingHint(RenderingHints.KEY_ANTIALIASING, RenderingHints.VALUE_ANTIALIAS_OFF);
...
class MultiSelections {
	...
	public MultiSelections(int numberOfItems, PointSetVisualization mainWindow, SelectionManager manager) {
	...
	public final void setActiveSelection(int n) {
		...
		//mainWindow.toFront();  //March08 WPM selected out
	}
	...
	public final void setSelection(int n, BitSet s) {
		if (getUnused(n)) { // this is a virgin selection, so we will make a tree window for it
			int uN = getUserNumber(n);
			mainWindow.newSelection((BitSet) s.clone(),"Selection " + uN, n);
			markUsed(n);
		} else { // a tree window already exists; change it
			int uN = getUserNumber(n);
			mainWindow.updateSelection((BitSet) s.clone(),"Selection " + uN, n);
		}
	...
	/** Removes the active selection permanently.  If it used an interesting icon (numbers 0-5) that icon
	...
	public void removeActiveSelection() {
		if (!getUnused(activeSelection)) { // a tree window was made, get rid of it
			mainWindow.removeConsensusWindow("Selection " + getUserNumber(activeSelection), activeSelection);
		}
...
class PointIcons {
	 ...
	 *  icons. */
	public static final void draw(int n, Graphics g, Point p, int pointSize, boolean frameDot) {
		switch (n) {
		case DOT: drawDot(g,p,pointSize, frameDot); break;
		case SAMPLE: drawSampled(g,p, pointSize); break;
	...
	private static final void drawDot(Graphics g, Point p, int pointSize, boolean frameDot) {
	...
	private static final void drawSampled(Graphics g, Point p, int pointSize) {
		int ps = pointSize+2;
		g.drawOval(p.x - ps, p.y - ps, 2 * ps + 1, 2 * ps + 1);
	}
